...
import { CreateUserBodyType, GetUsersQueryType, UpdateUserBodyType } from 'src/routes/user/user.dto';
...
export class UserService {
    constructor(
        ...
        private sharedUserRepository: UserRepository,
        private sharedRoleRepository: RoleRepo,
    ) {}
    ...
    async create({
        ...
        try {
            ...
            // Hash the password
            const hashedPassword = await this.hashingService.hash(data.password as string);
    ...
    async update({
        ...
        try {
            ...
            const updatedUser = await this.sharedUserRepository.update(id, data);
            return updatedUser;
        } catch (error) {
    ...
    private async getRoleIdByUserId(userId: number) {
        const currentUser = await this.sharedUserRepository.findById(userId);
        if (!currentUser) {
            throw NotFoundRecordException;
        }
